...
import { getUserDetails, logoutUser } from '../utils/authHandler'

const Sidebar = () => {
  const navigate = useNavigate()
  const user = getUserDetails()
  const logout = () => {
    // logout and move to root
    logoutUser()
    navigate('/')
  }
    ...
    <div className="border-end sidenav" id="sidebar-wrapper">
      ...
      <PerfectScrollbar className="sidebar-items">
        <ul className="list-unstyled ps-0">
          ...
          <li className="mb-1">
            <Link tag="a" className="" to="/workstations">
              <i className="fa fa-desktop"></i> Workstations
            </Link>
          </li>
          <hr className="dropdown-divider" />
          <li className="mb-1">
